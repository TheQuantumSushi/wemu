...
def emulate_asm_btb_assign(in1, debug=False):
    code = get_asm_btb_assign(in1)
    loader = AsmLoader(code)
    emulator = MuWMEmulator(name='btb_assign', loader=loader, debug=debug)
...
def emulate_asm_btb_or(in1, in2, debug=False):
    ...
    emulator.logger.log(f"Starting emulation of BTB-OR({in1}, {in2})...")
...
def emulate_asm_btb_and(in1, in2, debug=False):
    ...
    emulator.logger.log(f"Starting emulation of BTB-AND({in1}, {in2})...")
...
def emulate_asm_btb_and_or(in1, in2, in3, debug=False):
    ...
    emulator.logger.log(f"Starting emulation of BTB-AND-OR({in1}, {in2}, {in3})...")
